...
export async function gsPathToDownloadUrl(stored?: string): Promise<string | undefined> {
  ...
  try {
    console.debug(
      "[gsPathToDownloadUrl] attempting getDownloadURL for objectPath:",
      objectPath,
      "configuredBucket:",
      configuredBucket,
      "explicitBucket:",
    ...
    console.warn(
      "[gsPathToDownloadUrl] getDownloadURL failed for",
      objectPath,
      "error:",
  ...
  try {
    ...
    console.debug("[gsPathToDownloadUrl] metadata retrieved:", {
    ...
    if (rawToken) {
      ...
      if (bucketToUse) {
        ...
        console.warn(
          "[gsPathToDownloadUrl] token present but no bucket available to build REST URL",
        )
      }
    } else {
    ...
    console.warn(
      "[gsPathToDownloadUrl] getMetadata failed for",
      objectPath,
      "error:",
      metaErr?.code ?? metaErr?.message ?? metaErr,
    )
  }
  ...
  if (explicitBucket) {
    try {
      ...
      console.debug(
        "[gsPathToDownloadUrl] trying REST URL without token for explicit bucket:",
        trial,
      )
      ...
      try {
        ...
        if (res.ok) {
          console.debug(
            "[gsPathToDownloadUrl] HEAD succeeded on REST URL without token -> returning:",
            trial,
          )
          return trial
        } else {
