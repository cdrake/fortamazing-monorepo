 ...
 */
async function startAuthFlow(authUrl: string, returnUrl: string, timeoutMs = 60000) {
  ...
  console.log("[SocialAuth] AuthSession unavailable — using WebBrowser + Linking fallback")
 ...
 * GOOGLE SIGN-IN
 */
export async function signInWithGoogleAsync(): Promise<{ uid: string; idToken: string } | null> {
  ...
  if (Platform.OS === "web") {
    try {
      ...
      const { getAuth, signInWithPopup, GoogleAuthProvider: GProvider } = require("firebase/auth")
 ...
 * FACEBOOK SIGN-IN
 */
export async function signInWithFacebookAsync(): Promise<{ uid: string; idToken: string } | null> {
